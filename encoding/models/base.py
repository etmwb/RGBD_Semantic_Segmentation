...
class BaseNet(nn.Module):
    def __init__(self, nclass, backbone, aux, se_loss, dilated=True, norm_layer=None,
        ...
        if backbone == 'resnet50':
            ...
            self.pretrained = resnet.resnet152(pretrained=True, dilated=dilated,
                                               norm_layer=norm_layer, root=root)
        elif backbone == 'depth_resnet50':
            self.pretrained = resnetd.resnet50(pretrained=False, dilated=dilated,
                                              norm_layer=norm_layer, root=root)
        elif backbone == 'depth_resnet101':
            self.pretrained = resnetd.resnet101(pretrained=False, dilated=dilated,
                                               norm_layer=norm_layer, root=root)
        elif backbone == 'depth_resnet152':
            self.pretrained = resnetd.resnet152(pretrained=False, dilated=dilated,
                                               norm_layer=norm_layer, root=root)
        else:
    ...
    def depth_base_forward(self, x, depth):
        x = self.pretrained.conv1(x)
        x = self.pretrained.bn1(x)
        x = self.pretrained.relu(x)
        x = self.pretrained.maxpool(x)
        if self.pretrained.deep_base: 
            depth = self.pretrained.downsample3X3(self.pretrained.downsample3X3(depth))
        else: 
            depth = self.pretrained.downsample3X3(self.pretrained.downsample7X7(depth))

        c1 = self.pretrained.layer1(x)
        c2 = self.sequ_forward(self.pretrained.layer2, c1, depth)
        depth = self.pretrained.downsample3X3(depth)
        c3 = self.sequ_forward(self.pretrained.layer3, c2, depth)
        c4 = self.sequ_forward(self.pretrained.layer4, c3, depth)

        return c1, c2, c3, c4

    def sequ_forward(self, m, x, depth): 
