#include <vector>
#include "cuda/depth_deform_im2col_cuda.cuh"
...
at::Tensor
depth_deform_conv_cuda_forward(const at::Tensor &input, 
                    const at::Tensor &depth, 
                    const at::Tensor &depth_offset,
                    ...
                    const int im2col_step)
{
    ...
    AT_ASSERTM(depth.type().is_cuda(), "depth must be a CUDA tensor");
    AT_ASSERTM(depth_offset.type().is_cuda(), "depth_offset must be a CUDA tensor");
    ...
    const int depth_height = depth.size(2);
    const int depth_width = depth.size(3); 
    const int depth_offset_height = depth_offset.size(2);
    const int depth_offset_width = depth_offset.size(3);
    ...
    AT_ASSERTM(height == depth_height && width == depth_width,
               "Input shape and depth shape wont match: (%d x %d vs %d x %d).", height, width, depth_height, depth_width); 
    ...
    AT_ASSERTM(height_out == depth_offset_height && width_out == depth_offset_width,
               "Out shape and depth offset shape wont match: (%d x %d vs %d x %d).", height_out, width_out, depth_offset_height, depth_offset_width);
    ...
    const int per_depth_size = depth_height * depth_width; 
    const int per_depth_offset_size = depth_offset.size(1) * depth_offset.size(2) * depth_offset.size(3);
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "depth_deform_conv_forward_cuda", ([&] {
            depth_deformable_im2col_cuda(at::cuda::getCurrentCUDAStream(),
                                             input.data<scalar_t>() + n * im2col_step_ * per_input_size, 
                                             depth.data<scalar_t>() + n * im2col_step_ * per_depth_size, 
                                             depth_offset.data<int>() + n * im2col_step_ * per_depth_offset_size,
...
std::vector<at::Tensor> depth_deform_conv_cuda_backward(const at::Tensor &input, 
                                             const at::Tensor &depth, 
                                             const at::Tensor &depth_offset,
                                             const at::Tensor &weight,
                                             const at::Tensor &bias,
                                             ...
                                             const int im2col_step)
{
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "depth_deform_conv_backward_cuda", ([&] {
            depth_deformable_col2im_coord_cuda(at::cuda::getCurrentCUDAStream(),
                                                   columns.data<scalar_t>(),
                                                   input.data<scalar_t>() + n * im2col_step_ * per_input_size,
                                                   depth.data<scalar_t>() + n * im2col_step_ * per_depth_size, 
                                                   depth_offset.data<int>() + n * im2col_step_ * per_depth_offset_size,
            ...
            depth_deformable_col2im_cuda(at::cuda::getCurrentCUDAStream(),
                                             columns.data<scalar_t>(),
                                             depth.data<scalar_t>() + n * im2col_step_ * per_depth_size, 
                                             depth_offset.data<int>() + n * im2col_step_ * per_depth_offset_size,
                                             offset.data<scalar_t>() + n * im2col_step_ * per_offset_size,
