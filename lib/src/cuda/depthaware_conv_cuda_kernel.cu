#include <vector>
#include "cuda/depthaware_im2col_cuda.cuh"
                    ...
                    const int im2col_step)
{
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "depthaware_conv_forward_cuda", ([&] {
            depthaware_im2col_cuda(at::cuda::getCurrentCUDAStream(),
                     ...
                     const int im2col_step) 
{
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "depthaware_conv_backward_cuda", ([&] {
            // gradient w.r.t. input data
            depthaware_col2im_cuda(at::cuda::getCurrentCUDAStream(),
                                             columns.data<scalar_t>(),
                                             depth.data<scalar_t>() + n * im2col_step_ * per_depth_size, 
                                             batch_n, channels, height, width,
                                             height_out, width_out, kernel_h, kernel_w,
