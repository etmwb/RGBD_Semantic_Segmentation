...
from modules.depth_deform_conv import DepthDeformConv, _DepthDeformConv, DepthDeformConvPack, DepthOffset
...
def check_ddconv_im2col_step_forward():
    conv_offset = nn.Conv2d(inC, deformable_groups * 2 * kH * kW,
                            kernel_size=(kH, kW),
                            stride=(1, 1),
                            padding=(1, 1),
    ...
    input = torch.randn(N, inC, inH, inW).cuda()
    depth = torch.randn(N, 1, inH, inW).cuda()
    offset = conv_offset(input)
    mask = conv_mask(input)
    groups = 2

    dcn1 = DepthDeformConv(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=1).cuda()

    dcn2 = DepthDeformConv(inC, outC, (kH, kW), 
        stride=1, padding=1, dilation=1, 
        groups=groups, 
        deformable_groups=deformable_groups,
        im2col_step=2).cuda()
    dcn1.weight = dcn2.weight
    dcn1.bias = dcn2.bias
    output1 = dcn1(input, depth, offset, mask)
    output2 = dcn2(input, depth, offset, mask)

    d = (output1 - output2).abs().max()
    if d < 1e-10:
        print('ddconv im2col_step forward passed with {}'.format(d))
    else:
        print('ddconv im2col_step forward failed with {}'.format(d))
        print(output1)
        print(output2)
        print((output1 - output2).abs())
...
def check_ddconv_im2col_step_backward():
    stride = 1
    padding = 1
    groups = 2
    dilation = 1
    ...
    depth = torch.rand(N, 1, inH, inW).cuda() * 0.01
    ...
    output1 = _DepthDeformConv(input, depth, depth_offset, offset, mask, weight, bias, stride, padding, dilation, groups, deformable_groups, 2)
    targert = torch.rand(*output1.size()).cuda()
    error = (targert - output1).mean()
    error.backward(retain_graph=True)
    input_grad = input.grad.clone()
    ...
    weight_grad = weight.grad.clone()
    bias_grad = bias.grad.clone()
    output2 = _DepthDeformConv(input, depth, depth_offset, offset, mask, weight, bias, stride, padding, dilation, groups, deformable_groups, 1)
    error2 = (targert - output2).mean()
    error.backward()
    print((output1 - output2).abs().max())
    input_grad_err = (input.grad - 2 * input_grad).abs().max() 
    ...
    grad_err = input_grad_err + offset_grad_err + mask_grad_err + weight_grad_err + bias_grad_err
    if grad_err < 1e-7:
        print("ddconv im2col_step backward passed with {}".format(grad_err))
    else:
        print("ddconv im2col_step backward failed with {}".format(grad_err))
...
def example_ddconv():
    input = torch.randn(2, 64, 128, 128).cuda()
    depth = torch.randn(2, 1, 128, 128).cuda()
    # wrap all things (offset and mask) in DCN
    dcn = DepthDeformConvPack(64, 128, kernel_size=(3, 3), stride=1,
              padding=1, deformable_groups=2).cuda()
    # print(dcn.weight.shape, input.shape)
    output = dcn(input, depth)
    targert = output.new(*output.size())
    targert.data.uniform_(-0.01, 0.01)
    error = (targert - output).mean()
    error.backward()
...
if __name__ == '__main__':
    ...
    check_ddconv_im2col_step_forward()
    check_ddconv_im2col_step_backward()
    ...
    if inC == outC:
        ...
        # check_ddconv_zero_offset_identify()
