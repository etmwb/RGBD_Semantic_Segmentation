...
from ..functions.sample_conv_func import SampleConvFunction
...
class SampleConv(nn.Module):
    ...
    def forward(self, input, depth, offset, mask):
        ...
        return SampleConvFunction.apply(input, depth, depth_offset, offset, mask,
                                                   self.weight,
                                                   self.bias,
                                                   self.stride,
                                                   self.padding,
...
_SampleConv = SampleConvFunction.apply

class SampleConvPack(SampleConv):

    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding,
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1, fr_fa=False):
        super(SampleConvPack, self).__init__(in_channels, out_channels,
